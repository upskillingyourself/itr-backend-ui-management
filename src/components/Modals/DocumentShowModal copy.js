...
const DocumentShowModal = ({
    ...
    const fetchDocuments = async () => {
      try {
        const response = await axios.get(
          "https://api.toratax.com/toratax/rest/v1.0/documenttypes"
        );
  ...
  const getDocumentDetails = (docTypeId) => {
    ...
    if (permanentDataDetails) {
      details = permanentDataDetails.permanentDocuments.find(
        (permDoc) => permDoc.documentTypeId === docTypeId
      );
    }
    if (!details && yearlyDataDetails) {
      yearlyDataDetails.forEach((yearlyData) => {
        if (!details) {
          details = yearlyData.documentDetails.find(
            (yearlyDoc) => yearlyDoc.documentTypeId === docTypeId
          );
        }
      });
    }
              ...
              if (matchedDocument) {
                return (
                  ...
                  >
                    ...
                    <td>
                      <div
                        className="text-center gap-2 text-white bg-success px-1 py-1 rounded-pill"
                        role="button"
                        onClick={() =>
                          handleDownload(
                            matchedDocument.documentTypeId,
                            matchedDocument.documentId,
                            matchedDocument.documentName,
                            matchedDocument.documentPath
                          )
                        }
                      >
                        Download &nbsp;
                ...
                return (
                  <tr key={doc.documentTypeId} style={rowStyle}>
                    <td className="text-capitalize fw-normal">
                      {doc.documentName}
                    </td>
                    <td>
                      <div
